...
struct AnimatedStatsView: View {
    ...
    private var userTasks: [HouseholdTask] {
        guard let currentUser = authManager.currentUser else { return [] }
        return (currentUser.assignedTasks?.allObjects as? [HouseholdTask]) ?? []
    }
    ...
    private var completionPercentage: Double {
        ...
        return Double(tasksCompletedToday) / Double(max(totalTasksToday, tasksCompletedToday))
    }
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 20) {
            HStack {
                ...
                PulsingDotIndicator(color: completionPercentage >= 1.0 ? .green : .orange)
            }
            ...
            HStack(spacing: 20) {
                ...
                ZStack {
                    ...
                    VStack(spacing: 2) {
                        ...
                        Text("of \(max(totalTasksToday, tasksCompletedToday))")
                            .font(.system(.caption2, design: .rounded, weight: .medium))
                            .foregroundColor(.secondary)
                    }
                }
                    ...
                    HStack(spacing: 8) {
                        ...
                        VStack(alignment: .leading, spacing: 2) {
                            Text("Tasks Done")
                                .font(.system(.caption, design: .rounded, weight: .medium))
                                .foregroundColor(.secondary)
                            Text("\(tasksCompletedToday) completed")
                    ...
                    HStack(spacing: 8) {
                        ZStack {
                            Image(systemName: "flame.fill")
                                ...
                                .font(.title3)
                                .scaleEffect(streakFlameAnimation)
                        }
                        VStack(alignment: .leading, spacing: 2) {
                            Text("Current Streak")
                                .font(.system(.caption, design: .rounded, weight: .medium))
        ...
        .onAppear {
            // Animate streak flame - limited to prevent performance issues
            withAnimation(.easeInOut(duration: 1.2)) {
                streakFlameAnimation = 1.15
            }
            
            // Progress ring animation
            withAnimation(.spring(response: 1.0, dampingFraction: 0.8).delay(0.3)) {
                progressRingAnimation = completionPercentage
            }
        }
    }
}
