...
class PerformanceManager: ObservableObject {
    ...
    private func setupPerformanceMonitoring() {
        ...
        memoryTimer?.invalidate() // Clean up any existing timer first
        memoryTimer = nil
    ...
    func performBatchOperations<T: NSManagedObject>(_ operations: @escaping (NSManagedObjectContext) throws -> [T]) {
        // Capture the view context while we're on MainActor
        let viewContext = PersistenceController.shared.container.viewContext
        let backgroundContext = PersistenceController.shared.newBackgroundContext()
        
        // Now use Task.detached with the captured contexts
        Task.detached {
            backgroundContext.perform {
                do {
                    ...
                    // Merge changes to main context using the captured viewContext
                    DispatchQueue.main.async {
    ...
    func scheduleBackgroundCleanup() {
        // ✅ FIX: Use DispatchQueue instead of Task.detached for iOS 17 compatibility
        DispatchQueue.global(qos: .background).async { [weak self] in
            self?.cleanupOldData()
    ...
    // ✅ FIX: Mark as nonisolated since these methods work with background contexts
    nonisolated private func cleanupOldData() {
        // ✅ FIX: Use Task with MainActor to safely access shared
        Task { @MainActor in
            ...
            Task.detached {
                ...
                let taskRequest: NSFetchRequest<NSFetchRequestResult> = NSFetchRequest(entityName: "HouseholdTask")
                ...
                do {
                    ...
                    NSManagedObjectContext.mergeChanges(fromRemoteContextSave: changes, into: [persistenceController.container.viewContext])
    ...
    // ✅ FIX: Mark as nonisolated since this method works with background contexts
    nonisolated private func optimizeDatabase() {
        // ✅ FIX: Use Task with MainActor to safely access shared
        Task { @MainActor in
            let persistenceController = PersistenceController.shared
