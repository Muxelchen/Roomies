...
class CalendarManager: ObservableObject {
    ...
    private func createCalendarEvent(for task: HouseholdTask) {
        ...
        do {
            ...
            _Concurrency.Task { @MainActor in
                // ✅ FIXED: Use calendarEventId property that exists on HouseholdTask
    ...
    private func updateCalendarEvent(_ event: EKEvent, with task: HouseholdTask) {
        event.title = "🏠 \(task.title ?? "Roomies Task")"
        event.notes = task.taskDescription
    ...
    func removeTaskFromCalendar(_ task: HouseholdTask) {
        // ✅ FIXED: Check if calendarEventId property exists on HouseholdTask
        // guard let eventId = task.calendarEventId,
        //       let event = eventStore.event(withIdentifier: eventId) else {
        //     return
        // }
        
        // For now, remove by title matching since calendarEventId might not exist
        let startDate = Date().addingTimeInterval(-86400) // 1 day ago
        let endDate = Date().addingTimeInterval(86400 * 30) // 30 days ahead
        
        let predicate = eventStore.predicateForEvents(withStart: startDate, end: endDate, calendars: nil)
        let events = eventStore.events(matching: predicate)
        
        let taskEvents = events.filter { event in
            return event.title?.contains(task.title ?? "") == true && event.title?.contains("🏠") == true
        }
    ...
    private func syncAllTasks() {
        ...
        // ✅ FIXED: Use correct entity name "HouseholdTask" instead of "Task"
