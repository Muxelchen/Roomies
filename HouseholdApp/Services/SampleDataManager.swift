...
class SampleDataManager {
    ...
    private func createSampleUsers(context: NSManagedObjectContext, household: Household) -> [User] {
        ...
        for (index, name) in userNames.enumerated() {
            ...
            // Create membership (remove isActive since it doesn't exist in the model)
    ...
    private func createSampleTasks(context: NSManagedObjectContext, household: Household, users: [User]) {
        ...
        for (title, description, points, priority) in taskData {
            ...
            // Randomly assign to users (remove assignedDate since it doesn't exist in the model)
            if Bool.random() {
    ...
    private func createSampleRewards(context: NSManagedObjectContext, household: Household) {
        ...
        for (name, description, cost, _) in rewardData {
    ...
    private func createSampleBadges(context: NSManagedObjectContext, users: [User]) {
        // Badge functionality removed - not available in simplified Roomies Core Data Model
        // Badges are now handled as simple notifications through GameificationManager
        LoggingManager.shared.info("Badge creation skipped - using simplified notification system", category: LoggingManager.Category.coreData.rawValue)
    }
