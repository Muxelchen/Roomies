...
class NotificationManager: NSObject, ObservableObject {
    ...
    func scheduleTaskReminder(for task: Task) {
        ...
        // ✅ FIX: Validate trigger date is in future
        guard triggerDate > Date() else {
            LoggingManager.shared.debug("Trigger date is in the past, scheduling immediate notification", category: LoggingManager.Category.notifications.rawValue)
            // Schedule for 1 minute from now if trigger would be in past
            let immediateTrigger = UNTimeIntervalNotificationTrigger(timeInterval: 60, repeats: false)
            ...
            UNUserNotificationCenter.current().add(request) { error in
                if let error = error {
                    LoggingManager.shared.error("Error scheduling immediate notification", category: LoggingManager.Category.notifications.rawValue, error: error)
                }
            }
            return
        }
