...
class GameificationManager: ObservableObject {
    ...
    func checkForNewBadges(user: User, context: NSManagedObjectContext) {
        ...
        if completedTasks >= 10 && !existingBadgeTypes.contains("tasks_10") {
            awardBadge(to: user, type: "tasks_10", name: "Organizer", description: "10 tasks completed", icon: "checkmark.seal.fill", context: context)
        }
        
        if completedTasks >= 50 && !existingBadgeTypes.contains("tasks_50") {
            awardBadge(to: user, type: "tasks_50", name: "Household Pro", description: "50 tasks completed", icon: "house.circle.fill", context: context)
        }
