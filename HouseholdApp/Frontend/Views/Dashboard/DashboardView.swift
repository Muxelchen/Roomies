        ...
        VStack(alignment: .leading, spacing: 12) {
            HStack {
                VStack(alignment: .leading) {
                    Text("Hallo, \(currentUserName)!")
                        .font(.title2)
                        .fontWeight(.semibold)
                    
                    Text("Hier ist deine heutige Übersicht")
                        .font(.subheadline)
...
struct RecentAchievementsCardView: View {
    var body: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            HStack(spacing: 12) {
                BadgeView(iconName: "star.fill", name: "Aufsteiger", color: .yellow)
                BadgeView(iconName: "flame.fill", name: "Streak-Master", color: .orange)
                BadgeView(iconName: "checkmark.seal.fill", name: "Aufräumer", color: .green)
            }
        }
        .padding()
        .background(Color(UIColor.systemBackground))
    ...
    var body: some View {
        HStack(spacing: 12) {
            ...
            VStack(alignment: .leading, spacing: 4) {
                ...
                // Progress Bar
                HStack {
                    Text("Fortschritt: \(challenge.progress)/\(challenge.target)")
                        .font(.caption)
                        .foregroundColor(.secondary)
