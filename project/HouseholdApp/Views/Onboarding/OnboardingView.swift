...
struct RoomiesOnboardingBackground: View {
    let currentPage: Int
    @State private var floatingElements: [FloatingOnboardingElement] = []
    ...
    private func generateFloatingElements() {
        let emojis = ["🏠", "🎁", "🏆", "⭐", "✨", "🎉", "🌟", "💫"]
        floatingElements = (0..<8).map { index in
            FloatingOnboardingElement(
                id: index,
...
struct FloatingOnboardingElement {
    let id: Int
    let x: CGFloat
    let y: CGFloat
    let size: CGFloat
